...
class TestRunStep:
    ...
    def test_rosters_calls_backfill(
        self, mock_fetch: MagicMock, mock_backfill: MagicMock, db_path: Path
    ) -> None:
    ...
    @patch("pipeline.fetch_goalie_game_log")
    @patch("pipeline.save_skater_stats")
    @patch("pipeline.fetch_skater_game_log")
    def test_gamelogs(
        self,
        mock_skater_fetch: MagicMock,
        mock_skater_save: MagicMock,
        mock_goalie_fetch: MagicMock,
        mock_goalie_save: MagicMock,
        mock_time: MagicMock,
        db_path: Path,
        db: sqlite3.Connection,
    ) -> None:
        """run_step('gamelogs') fetches skater and goalie logs."""
        _insert_test_players(db)
        db.close()

        mock_skater_fetch.return_value = [
            {"game_date": "2026-01-01", "toi": 1200, "goals": 1,
             "assists": 0, "points": 1, "plus_minus": 0, "pim": 0,
             "shots": 3, "hits": 1, "blocks": 0,
             "powerplay_goals": 0, "powerplay_points": 0,
             "shorthanded_goals": 0, "shorthanded_points": 0},
        ]
        mock_goalie_fetch.return_value = [
            {"game_date": "2026-01-01", "toi": 3600, "saves": 30,
             "goals_against": 2, "shots_against": 32,
        ...
        assert mock_skater_fetch.call_count == 7
        assert mock_goalie_fetch.call_count == 2

    @patch("pipeline.time_module")
    @patch("pipeline.save_goalie_stats")
    @patch("pipeline.save_skater_stats")
    @patch("pipeline.fetch_player_landing")
    def test_seasontotals(
        ...
        """run_step('seasontotals') calls fetch_player_landing per player."""
        _insert_test_players(db)
        db.close()
            ...
            "featuredStats": {
                "regularSeason": {
                    "subSeason": {
                        ...
                        "plusMinus": 10, "pim": 12, "shots": 200,
                        "powerPlayGoals": 8, "powerPlayPoints": 20,
                        "shorthandedGoals": 0, "shorthandedPoints": 0,
                        "saves": 900, "goalsAgainst": 100,
                        "shotsAgainst": 1000, "wins": 20,
                        "losses": 10, "otLosses": 3, "shutouts": 2,
                    }
                }
            }
        }
        ...
        assert mock_landing.call_count == 9
