...
# =============================================================================
# Season Month Ranges Tests (4 tests)
# =============================================================================


class TestSeasonMonthRanges:
    """Tests for _season_month_ranges helper."""

    def test_standard_season(self) -> None:
        """20252026 season returns 7 month ranges Oct-Apr."""
        ranges = _season_month_ranges("20252026")
        assert len(ranges) == 7
        assert ranges[0] == ("2025-10-01", "2025-10-31")
        assert ranges[1] == ("2025-11-01", "2025-11-30")
        assert ranges[2] == ("2025-12-01", "2025-12-31")
        assert ranges[3] == ("2026-01-01", "2026-01-31")
        assert ranges[4] == ("2026-02-01", "2026-02-28")
        assert ranges[5] == ("2026-03-01", "2026-03-31")
        assert ranges[6] == ("2026-04-01", "2026-04-30")

    def test_leap_year_february(self) -> None:
        """Season crossing a leap year (Feb 2024 has 29 days)."""
        ranges = _season_month_ranges("20232024")
        feb_range = ranges[4]  # Index 4 = February
        assert feb_range == ("2024-02-01", "2024-02-29")

    def test_non_leap_year_february(self) -> None:
        """Season crossing a non-leap year (Feb 2025 has 28 days)."""
        ranges = _season_month_ranges("20242025")
        feb_range = ranges[4]
        assert feb_range == ("2025-02-01", "2025-02-28")

    def test_year_parsing(self) -> None:
        """Start and end years are correctly parsed from season string."""
        ranges = _season_month_ranges("20192020")
        assert ranges[0][0].startswith("2019-")
        assert ranges[6][0].startswith("2020-")


# =============================================================================
# Bulk Skater Gamelogs Monthly Chunking Tests (4 tests)
# =============================================================================


class TestFetchAllSkaterGamelogsBulk:
    """Tests for fetch_all_skater_gamelogs_bulk monthly chunking."""
    ...
    @patch("fetchers.nhl_api._paginate_stats_api")
    @patch("fetchers.nhl_api.time.sleep")
    def test_date_filter_in_cayenne_exp(
        self, mock_sleep: MagicMock, mock_paginate: MagicMock
    ) -> None:
        """Each call includes date range in cayenneExp parameter."""
        mock_paginate.return_value = []
        fetch_all_skater_gamelogs_bulk("20252026")

        # Check the first call's params contain the October date range
        first_call_args = mock_paginate.call_args_list[0]
        params = first_call_args[0][2]  # positional arg index 2 = params dict
        cayenne = params["cayenneExp"]
        assert '2025-10-01' in cayenne
        assert '2025-10-31' in cayenne

    @patch("fetchers.nhl_api._paginate_stats_api")
    @patch("fetchers.nhl_api.time.sleep")
    def test_joins_summary_and_realtime(
        self, mock_sleep: MagicMock, mock_paginate: MagicMock
    ) -> None:
        """Summary and realtime rows are joined by (playerId, gameId)."""
        summary_row = {
            "playerId": 8478402, "gameId": 2024020001,
            "gameDate": "2025-10-15", "goals": 2, "assists": 1,
            "points": 3, "plusMinus": 1, "penaltyMinutes": 0,
            "shots": 5, "timeOnIcePerGame": 1350,
    ...
    @patch("fetchers.nhl_api._paginate_stats_api")
    @patch("fetchers.nhl_api.time.sleep")
    def test_empty_months_handled(
        self, mock_sleep: MagicMock, mock_paginate: MagicMock
    ) -> None:
        """Months with no games return 0 rows without error."""
        mock_paginate.return_value = []
        result = fetch_all_skater_gamelogs_bulk("20252026")
        assert result == []
        assert mock_paginate.call_count == 21
...
# =============================================================================
# Discover Missing Players Tests (5 tests)
# =============================================================================


class TestDiscoverMissingPlayers:
    """Tests for discover_missing_players function."""

    @patch("fetchers.nhl_api.time.sleep")
    @patch("fetchers.nhl_api._api_get")
    @patch("fetchers.nhl_api._paginate_stats_api")
    def test_discovers_missing_skater(
        self,
        ...
        """Player in Stats API but not in DB gets discovered and upserted."""
        # Seed one known player
        upsert_player(db, {
        ...
        # Stats API returns known + unknown player
        mock_paginate.side_effect = [
            [{"playerId": 8478402}, {"playerId": 9999999}],  # skaters
            [],  # goalies
        ]

        # Player landing for unknown player
        mock_landing_resp = Mock()
        mock_landing_resp.json.return_value = {
            "firstName": {"default": "Clayton"},
            "lastName": {"default": "Keller"},
            "currentTeamAbbrev": "UTA",
            "position": "LW",
        }
        mock_api_get.return_value = mock_landing_resp

        discovered = discover_missing_players(db, "20252026")

        assert discovered == 1
        row = db.execute(
            "SELECT full_name, team_abbrev, position FROM players WHERE id = 9999999"
        ).fetchone()
        assert row["full_name"] == "Clayton Keller"
        assert row["team_abbrev"] == "UTA"
        assert row["position"] == "LW"

    @patch("fetchers.nhl_api.time.sleep")
    @patch("fetchers.nhl_api._api_get")
    @patch("fetchers.nhl_api._paginate_stats_api")
    def test_skips_already_known_players(
        self,
        mock_paginate: MagicMock,
        mock_api_get: MagicMock,
        mock_sleep: MagicMock,
        ...
        """Players already in the DB are not re-fetched."""
        upsert_player(db, {
            "id": 8478402, "full_name": "Connor McDavid",
            "team_abbrev": "EDM",
        })
    ...
    @patch("fetchers.nhl_api._api_get")
    @patch("fetchers.nhl_api._paginate_stats_api")
    def test_discovers_missing_goalie(
        self,
        mock_paginate: MagicMock,
        mock_api_get: MagicMock,
        mock_sleep: MagicMock,
        ...
        """Goalie in Stats API but not in DB gets discovered."""
        mock_paginate.side_effect = [
            [],  # skaters
            [{"playerId": 8888888}],  # goalies
        ]
        ...
        assert discovered == 1
        row = db.execute("SELECT full_name FROM players WHERE id = 8888888").fetchone()
    ...
    def test_continues_on_individual_failure(
        ...
        """One player landing fails, others still discovered."""
        mock_paginate.side_effect = [
            [{"playerId": 1111111}, {"playerId": 2222222}],  # skaters
            [],  # goalies
        ]

        landing_ok = Mock()
        landing_ok.json.return_value = {
            "firstName": {"default": "Good"},
        ...
        mock_api_get.side_effect = [
            Exception("API error"),  # first player fails
            landing_ok,  # second succeeds
        ]
    ...
    @patch("fetchers.nhl_api.time.sleep")
    @patch("fetchers.nhl_api._paginate_stats_api")
    def test_empty_stats_api_no_calls(
        self,
        mock_paginate: MagicMock,
        mock_sleep: MagicMock,
        db: sqlite3.Connection,
    ) -> None:
        """Empty Stats API response results in 0 discoveries."""
        mock_paginate.side_effect = [[], []]
    ...
    def test_live_discovers_players(self, db: sqlite3.Connection) -> None:
