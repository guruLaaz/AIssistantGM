        ...
        for stats_group, schedule_group in zip(self._data["tables"], data[1]["tables"]):
            ...
            for stats_row, schedule_row in zip(stats_group["rows"], schedule_group["rows"]):
                if "posId" not in stats_row:
                    continue
                stuff = {"posId": stats_row["posId"], "statusId": stats_row.get("statusId"), "future_games": {}, "total_fantasy_points": None, "fantasy_points_per_game": None, "salary": None}
                if "scorer" in stats_row or stats_row["statusId"] == "1":
                    if "scorer" in stats_row:
                        ...
                        for header, cell in zip(stats_header, stats_row["cells"]):
                            # Check shortName for salary
                            if "shortName" in header and header["shortName"] == "Sal" and cell["content"]:
                                # Remove $ and commas, convert to float
                                salary_str = str(cell["content"]).replace("$", "").replace(",", "")
                                try:
                                    stuff["salary"] = float(salary_str)
                                except ValueError:
                                    stuff["salary"] = None

                            # Check sortKey for other stats
...
class RosterRow(FantraxBaseObject):
    ...
    Attributes:
        ...
        status_id (str | None): The status ID (1=Active, 2=Reserve, 3=Inj Res).
        ...
        salary (float | None): The Salary/Cap Hit for the Player in the RosterRow.
    ...
    def __init__(self, roster: Roster, data: dict) -> None:
        ...
        self.status_id: str | None = self._data.get("statusId")
        ...
        self.salary: float | None = self._data.get("salary")
