...
if TYPE_CHECKING:
    from aissistant_gm.fantrax.fantraxapi.objs import League
...
def get_player_profile(league: "League", player_id: str, team_id: str | None = None) -> dict:
    """Get player profile data.

    Note: This API method returns limited data. For player news, use getPlayerNews instead.

    Args:
        league: League instance
        player_id: Player ID (e.g., "004p4")
        team_id: Optional team ID (e.g., "5pr8olt0mbc80puf")

    Returns:
        dict: Player profile data
    """
    return request(league, Method("getPlayerProfile", playerId=player_id, teamId=team_id))


def get_player_news(league: "League", pool_type: str = "ALL") -> dict:
    """Get player news for a pool of players.

    Args:
        league: League instance
        pool_type: Type of player pool - "POOL" (my roster), "ALL" (all players),
                   or "WATCH_LIST" (watch list)

    Returns:
        dict: News data with 'stories' array containing player news
    """
    return request(league, Method("getPlayerNews", poolType=pool_type))
