...
from aissistant_gm.fantrax.fantraxapi import NotLoggedIn, NotTeamInLeague, api
...
class League:
    ...
    Attributes:
        ...
        my_team_id (str | None): The logged-in user's Team ID.
        my_team (Team | None): The logged-in user's Team object.
    """
    ...
    def my_team(self) -> Team | None:
        """Return the Team object for the logged-in user's team.

        Returns:
            Team | None: The user's Team object, or None if not determined.
        """
        if self.my_team_id and self.my_team_id in self.team_lookup:
